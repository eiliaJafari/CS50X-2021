...
def index():
    ...
    balance = rows[0]['cash']
    grandtotal = balance
    rows = db.execute("SELECT symbol FROM moamele WHERE user_id=:id", id=session["user_id"])
    symbols = []
    for row in rows:
    ...
    for i in range(len(symbols)):
        rows = db.execute("SELECT symbol, SUM(shares) FROM moamele WHERE user_id=:id AND symbol=:symbol",
...
def buy():
    ...
    if request.method == "POST":
        ...
        balance = cash - price
        symbol = quote['symbol']
        db.execute("INSERT INTO moamele (user_id, symbol, shares, price) VALUES (:user, :symbol, :shares, :price)",
...
def history():
    ...
    rows = db.execute("SELECT symbol, shares, price, time FROM moamele WHERE user_id=:id ORDER BY time DESC",
                      id=session["user_id"])
    print(rows)
...
def sell():
    ...
    if request.method == "POST":
        ...
        if shares < 1:
            return apology("shares should be a positive integer", 400)
        symbol = quote['symbol']
        rows = db.execute("SELECT SUM(shares) FROM moamele WHERE user_id = :id AND symbol = :symbol",
